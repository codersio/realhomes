...
if ( ! function_exists( 'inspiry_dashboard_customizer' ) ) {
	function inspiry_dashboard_customizer( WP_Customize_Manager $wp_customize ) {
		...
		/* Auto user/agent assignment controls in case the agent area if disabled */
		$wp_customize->add_setting(
			'realhomes_auto_user_agent_assignment',
			array(
				'type'              => 'option',
				'default'           => 'none',
				'transport'         => 'postMessage',
		...
		$wp_customize->add_control(
			'realhomes_auto_user_agent_assignment',
			array(
				'label'       => esc_html__( 'Control auto assignment of user/agent if related section is disabled.', 'framework' ),
				'description' => esc_html__( 'If agent is selected, but user agent sync is not enabled then user will be assigned instead. Display option will be similar to this setting as well.', 'framework' ),
				'type'        => 'radio',
				'section'     => 'inspiry_members_user_sync',
				'choices'     => array(
					'none'  => esc_html__( 'None', 'framework' ),
					'user'  => esc_html__( 'User', 'framework' ),
					'agent' => esc_html__( 'Agent', 'framework' )
				)
			)
		);
