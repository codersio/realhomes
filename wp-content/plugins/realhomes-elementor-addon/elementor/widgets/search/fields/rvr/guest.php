...
if ( is_array($search_fields_to_display) && in_array( 'guests', $search_fields_to_display ) ) {
    ...
    <div class="rhea_prop_search__option rhea_prop_search__select rhea_rvr_guests_field <?php echo esc_attr( $separator_class ) ?>"
		...
		if ( 'yes' === $settings['show_labels'] ) {
            ...
            <label class="rhea_fields_labels" for="select-status-<?php echo esc_attr( $the_widget_id ); ?>">
				<?php
				if ( ! empty( $settings['guests_label'] ) ) {
					echo esc_html( $settings['guests_label'] );
				} else {
					echo esc_html__( 'Guests', 'realhomes-elementor-addon' );
				}
				?>
            </label>
			<?php
        ...
        <span class="rhea_prop_search__selectwrap">
            <select name="guests" id="select-guests-<?php echo esc_attr( $the_widget_id ); ?>"
                    class="rhea_multi_select_picker show-tick"
                    data-size="<?php echo esc_attr($settings['rhea_dropdown_items_in'] ); ?>"
                    title="<?php echo esc_attr( $guests_placeholder_text ); ?>"
            >
                <?php
