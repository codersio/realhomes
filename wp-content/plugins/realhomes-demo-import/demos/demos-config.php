...
function rhdi_register_plugins( $plugins ): array {
	...
	if ( class_exists( 'RealHomes_Helper' ) ) {
		...
		if ( isset( $_GET['step'] ) && $_GET['step'] === 'import' && isset( $_GET['import'] ) ) {
			...
			if ( in_array( $demo_to_import, array( 'Vacation Rentals - Elementor', 'Vacation Rentals Ultra - Elementor' ) ) ) {
				$theme_plugins[] = [
...
function rhdi_after_import_setup( $selected_import ) {
	...
	} else if ( 'Vacation Rentals - Elementor' === $import_demo_name || 'Vacation Rentals Ultra - Elementor' === $import_demo_name ) {
	...
	if ( in_array( $import_demo_name, array( 'Single Property - Elementor', 'Condominium - Elementor' ) ) ) {
